xn.test.suite("Attribute Applier module tests", function(s) {
    s.tearDown = function() {
    ...
    s.test("Editable tests", function(t) {
        ...
        var util = rangy.AttributeApplier.util;
    ...
    s.test("isAppliedToRange tests", function(t) {
        var applier = rangy.createAttributeApplier("test");

        var testEl = document.getElementById("test");
        testEl.innerHTML = 'Test <span id="one" test="true">One</span> x <span id="two" test="true">Two <span id="three">Three</span> two</span> test';
    ...
    s.test("toggleRange simple test 1", function(t) {
        var applier = rangy.createAttributeApplier("test", true);
        var testEl = document.getElementById("test");
        testEl.innerHTML = 'Test <span id="one" test="true">One</span> test';
        ...
        t.assertEquals(testEl.firstChild.data, "Test One test");

        testEl.innerHTML = 'Test <span id="one" test="false">One</span> test';
        ...
        t.assertEquals(testEl.childNodes[1].getAttribute("test"), "true");
    ...
    s.test("toggleRange simple test 2", function(t) {
        ...
        var testEl = document.getElementById("test");
        testEl.innerHTML = 'Test <span id="one" test="true" other="true">One</span> test';
        var oneEl = document.getElementById("one");
        var range = rangy.createRangyRange();
        range.selectNodeContents(oneEl);
        applier.toggleRange(range);
        ...
        t.assertEquals(testEl.childNodes[1].tagName, "SPAN");
        t.assertEquals(testEl.childNodes[1].id, "one");
        t.assertEquals(testEl.childNodes[1].hasAttribute("other"), true);
        t.assertEquals(testEl.childNodes[1].childNodes.length, 1);
        t.assertEquals(testEl.childNodes[1].firstChild.data, "One");
        ...
        t.assertEquals(testEl.childNodes[1].tagName, "SPAN");
        t.assertEquals(testEl.childNodes[1].id, "one");
        t.assertEquals(testEl.childNodes[1].getAttribute("other"), "true");
        t.assertEquals(testEl.childNodes[1].getAttribute("test"), "true");
        t.assertEquals(testEl.childNodes[1].childNodes.length, 1);
        t.assertEquals(testEl.childNodes[1].firstChild.data, "One");
    });
    ...
    s.test("toggleRange nested in other class test", function(t) {
        ...
        var testEl = document.getElementById("test");
        testEl.innerHTML = 'Before <span id="one" other="true">One</span> after';
        var oneEl = document.getElementById("one");
        var range = rangy.createRangyRange();
        ...
        t.assertEquals(oneEl.getAttribute("other"), "true");
        t.assertEquals(oneEl.hasAttribute("test"), false);
        ...
        t.assertEquals(oneEl.childNodes[1].getAttribute("test"), "true");
    ...
    s.test("Test unapply to range spanning two blocks", function(t) {
        var applier = rangy.createAttributeApplier("c1", true);
...
/*
    ...
    s.test("Test multiple classes", function(t) {
        var applier1 = rangy.createAttributeApplier("c1"),
            applier2 = rangy.createAttributeApplier("c2");
...
*/
    ...
    s.test("Test issue 50 (Mac double click)", function(t) {
        var applier = rangy.createAttributeApplier("c1");
    ...
    s.test("Test whitespace 3 (ignorable whitespace, ignore option disabled)", function(t) {
        var applier = rangy.createAttributeApplier("c1", {ignoreWhiteSpace: false});
    ...
    s.test("Test whitespace 4 (pre whitespace between paras)", function(t) {
        var applier = rangy.createAttributeApplier("c1", {ignoreWhiteSpace: true});
    ...
    s.test("Test adding extra class with overlapping containers", function(t) {
        var applier = rangy.createAttributeApplier("c1", { elementProperties: { "className": "extra" } });
        var applier2 = rangy.createAttributeApplier("c2", { elementProperties: { "className": "extra" } });
    ...
    s.test("Issue 111 (extra option for useExistingElements)", function(t) {
        ...
        applier = rangy.createAttributeApplier("c1", {
    ...
    s.test("Merge error (issue 176)", function(t) {
        var applier = rangy.createAttributeApplier("one");
        var testEl = document.getElementById("test");
